            ...
            if EspPlayersToggle then
                if obj:IsA("Model") and obj:FindFirstChild("Head") then
                    for _, part in ipairs(obj:GetChildren()) do
                        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" and part.Name ~= "__HITBOX" then
                            part.Transparency = 0
                        end
                    end
        ...
        if v then
            for _, plotModel in pairs(workspace.Plots:GetChildren()) do
                for _, child in pairs(plotModel:GetDescendants()) do
                    if child:IsA("BasePart") and not string.find(string.lower(child.Name), "hitbox") then
