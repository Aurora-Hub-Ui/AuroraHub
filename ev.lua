...
local premium_users = { "Tgpeek1", "Technique12_12", "Vbn_bountyhunter", "iruzruz", "731niic", "RRQLEMONNl", "pedro377637", "blorospo", "flespos83", "prexos837", "polop7365", "Jaycol1", "NoSoyDekuGuys", "KandaKoe", "balle0704", "artile134", "urboyfiePoP", "Bva_Back", "Jinnxftw", "Zyxnn_18", "fanSukasusu", "tutioenRobloxgenial"}
...
local function getRootPosition(target)
    ...
    if target:IsA("Model") then
        ...
        local hitbox = target:FindFirstChild("Hitbox") or target:FindFirstChild("HumanoidRootPart")
...
local function ensureBillboard(obj)
    ...
    if not esp[obj].billboard then
        local head = obj:FindFirstChild("Head") or obj:FindFirstChild("Hitbox") or obj:FindFirstChildWhichIsA("BasePart")
        ...
        if data.billboard then
            ...
            if data.billboard.Enabled and myRoot then
                if data.studsLabel then
                    local dist = (myRoot.Position - worldPos).Magnitude
