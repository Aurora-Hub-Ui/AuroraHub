...
local function isPlayerObject(obj)
    return type(obj.Name) == "string" and not obj.Name:find(" ") and not obj:FindFirstChild("Revives")
end
...
-- Get ESP color based on type
local function getObjColor(obj)
    if isInjuredPlayer(obj) then return colors.injured end
    if isNextbotObject(obj) then return colors.nextbot end
    return colors.player
end

-- Remove all ESP data for object
...
-- Billboard GUI for names and studs
...
RunService.RenderStepped:Connect(function()
    local now = tick()
    if now - lR > rI then
        refreshAll() -- refresh all ESP objects periodically
        lR = now
    end
    ...
    for obj, data in pairs(esp) do
        ...
        -- Skip completely if object is behind camera
...
PlayersFolder.ChildAdded:Connect(function(child) task.wait(0.5); refreshAll() end)
PlayersFolder.ChildRemoved:Connect(function(child) removeESP(child) end)
